...
// Static buffers for RTT channels 1 and 2
static char _acUpBuffer1[1024];
static char _acUpBuffer2[1024];
...
void motor_init(void) {
    SEGGER_RTT_Init();
    SEGGER_RTT_ConfigUpBuffer(1, "Target_RPM", _acUpBuffer1, sizeof(_acUpBuffer1), SEGGER_RTT_MODE_NO_BLOCK_SKIP);
    SEGGER_RTT_ConfigUpBuffer(2, "Motor_Speed", _acUpBuffer2, sizeof(_acUpBuffer2), SEGGER_RTT_MODE_NO_BLOCK_SKIP);
...
void PI_update(void) {
     ...
     */
    uint16_t max_integral = 3200;
    if (error_integral < 0) {
        error_integral = 0;
    } else if (error_integral > max_integral) {
